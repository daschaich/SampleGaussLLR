...
Njacknife = 50; %Number of Jacknife blocks to estimate the stochastic error of a
Naverage = 10000;%100000; %Number of random numbers we sample to calculate the reweighted expectation value <<x-x0-0.5*delta>> 
...
%         a_i_del = a_i;
%         a_i_del(i) = [];
%         A(i) = 1/(Njacknife-1)*sum(a_i_del);
%         
%     end
%     
%     Astar = sum(A)/Njacknife;
%     
%     var(Eint)= (Njacknife-1)/Njacknife * sum(A-Astar)^2;
%     
%     standvar(Eint) = sqrt(var(Eint));
    
    
    T = sum(a)- a(1)/2 - a(Eint)/2;
    
    rho(Eint) = exp(delta*T);
