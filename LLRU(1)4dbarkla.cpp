...
int main()
{
	...
	for(int Eint=0;Eint<((Emax-Emin)/delta);Eint++) //scans through the energy intervals
	{
		...
		for(int jcount = 0;jcount<Njacknife;jcount++) //generates Njacknife values of a for Jacknife method to get error and average
		{
			...
			if(Eint == 0)
			{
				a_i[jcount] = 1+2*epsilon;//-1.0018+2*epsilon;
			...
			while(abs(a_i_new-a_i[jcount]) > epsilon) //only stop rob march if a doesnt change much anymore
			{
				...
				if(Einterval == false)
				{
					...
					{	
						//cout << "here Einterval false " << calcaction(theta,neighbour,beta,V)/6 << endl;
						metropolisupdate(theta,1.0,beta,neighbour,1,V);
						if(calcaction(theta,neighbour,beta,V)<=(x0[Eint]+delta) && calcaction(theta,neighbour,beta,V)>=x0[Eint])
						{
							Einterval=true;
							//cout << "found Energy interval  " << calcaction(theta,neighbour,beta,V)/6 <<  endl;
							myfileoutputinnerloop << "found Energy interval  " << calcaction(theta,neighbour,beta,V)/6 << " \n";
						}
					}
				}
				...
				*/
				...
				//cout << "RobMarchcount " <<  RobMarchcount << " Energy " <<  calcaction(theta,neighbour,beta,V)/6 << endl;
				myfileoutputinnerloop << "RobMarchcount " <<  RobMarchcount << " Energy " <<  calcaction(theta,neighbour,beta,V)/6 << " \n";
				...
				{
					Reweightexpect = Reweightexpect + calcaction(theta,neighbour,beta,V);
		...
		myfileoutput << "Reweightexpect: " << Reweightexpect << "a: " << a[Eint] << "Energyinterval: " << Eint*delta/6 <<"Energy: " << calcaction(theta,neighbour,beta,V)/6 << " \n";
		//cout << "Reweightexpect: " << Reweightexpect << endl;
		//cout << "a: " << a[Eint] << endl;
		//cout << "Energyinterval: " << Eint*delta/6 << endl;
		//cout << "Energy: " << calcaction(theta,neighbour,beta,V)/6 << endl;
		//cout << " " << endl;
			...
			for(int j=0;j<d;j++)
			{
				...
				else
				{
			 	...
			 	if(r<=rho)
			 	{
			 		save = theta[i][j];
			 		theta[i][j]=offer;
			 		if(calcaction(theta,neighbour,beta,V)<(E+delta) && calcaction(theta,neighbour,beta,V)>=E)
			 		{
