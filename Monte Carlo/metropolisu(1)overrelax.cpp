...
double staple2(double theta[V][d], int neighbour[V][2*d], int i, int j);
...
int main()
{
	...
	//cout << staple2(theta,neighbour,3,0) << endl;
	...
	for(int n=0;n<nsweeps;n++)
	{
		for(int i=0;i<V;i++)
		{
			for(int j=0;j<3;j++)
			{
				...
				theta[i][j] = 2*staple2(theta,neighbour,i,j)-theta[i][j];
			}
